# The Tutorial

Welcome. This tutorial keeps things deliberately simple: every example runs,
every chapter adds exactly one new idea, and every chapter ends with a small
tweak you make yourself.

You don't need to know how to program. You need curiosity and a willingness to
change one thing and click again.

## How to use it

Each chapter is a folder. Inside a chapter you'll find:

- a short **lesson note** (named after the chapter, like `Hello`) — read it first;
- one or more **action notes** (`.md` files) — the working examples;
- one or more **Exercises** at the end of the lesson — hands-on changes to try.

The rhythm for every chapter is the same:

1. Read the lesson note.
2. Open the action note and **run** it. See the result.
3. Do the Exercise: change one thing, run it again, see what changed.

That loop — *click, tweak, click* — is the whole point. It's cheap to re-run,
so play.

## The chapters

1. [[Hello]] — your first run, and your first look at a note's parts.
2. [[Variables]] — naming values with *Let … = …*.
3. [[Functions]] — defining your own reusable steps.
4. [[Composition]] — notes calling notes.
5. [[Conditionals]] — *If … Otherwise …*.
6. [[Loops]] — *For each … in …*.
7. [[Data]] — notes that hold values, and walking through lists.
8. [[Recursion]] — a note that calls itself.
9. [[Slots]] — letting the engine fill in a value for you with `{{ … }}`.

Chapter 9 is the one to look forward to: it's where you stop *typing* values and
start *describing* them. Write `{{ a calm shade of blue }}` and the engine fills
in the rest — plain English in, a real working value out.

Want the deep language reference instead of the gentle walk? That's the E--
language tutorial (coming later) — this one keeps things concrete.

**Next Lesson:** [[Hello]]
