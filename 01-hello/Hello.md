...
click re-derives the Recipe from those words via an AI. It may **snap to
something new** rather than change slightly, since prose is interpreted, not
copied. Want an exact string instead? Write it in the Recipe directly.

Now undo your AI re-derive: type `Return "hello, world".` back into the
Recipe and Forge once more.
